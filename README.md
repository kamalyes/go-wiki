...
GO-Wiki 是一个轻量级的企业知识分享与团队协同软件，可用于快速构建企业 Wiki 和团队知识分享平台。部署方便，使用简单，帮助团队构建一个信息共享、文档管理的协作环境。
</br>

[![stable](https://img.shields.io/badge/stable-stable-green.svg)](https://github.com/kamalyes/go-wiki/) 
[![license](http://img.shields.io/badge/license-MIT-red.svg?style=flat)](https://raw.githubusercontent.com/kamalyes/go-wiki/master/LICENSE)
...
打开 https://github.com/kamalyes/go-wiki/releases 找到对应平台的版本下载编译好的压缩包
...
- Linux 平台
    ...
    # https://github.com/kamalyes/go-wiki/releases 自行下载 wget http://
    # 解压到当前目录
    $ tar -zxvf go-wiki-linux-amd64.tar.gz
    # 进入程序安装目录
    $ cd install
    # 执行安装程序，默认端口为 8090，指定其他端口加参数 --port=8087
    $ ./install
    # 浏览器访问 http://ip:8090 进入安装界面，完成安装配置
    # Ctrl + C 停止 install 程序, 启动 GO-Wiki 系统
    $ cd ..
    $ ./go-wiki --conf conf/go-wiki.conf
    # 浏览器访问你监听的 ip 和端口
    # 开始 GO-Wiki 的使用之旅吧！
    ```
...
- Windows 平台
    ...
    # https://github.com/kamalyes/go-wiki/releases 自行下载
    # 手动解压到当前目录
    # 进入 install 目录
    # 双击点开 install.exe 文件
    # 浏览器访问 http://ip:8090 进入安装界面，完成安装配置
    ...
    $ 执行 go-wiki.exe --conf conf/go-wiki.conf
    # 浏览器访问你监听的 ip 和端口
    # 开始 GO-Wiki 的使用之旅吧！
    ```
...
- Docker 部署
    ...
    # docker run -d -p 8080:8081 -v /data/go-wiki/conf/:/opt/go-wiki/conf/ -v /data/go-wiki/data:/data/go-wiki/data/ --name go-wiki eahom/go-wiki:v0.1.7

    # 本地构建最新代码
    # 构建项目镜像
    # docker build -t go-wiki-image .
    # 新增配置文件，数据存放目录以及Mysql数据库配置在mm-wiki.conf配置文件中设置
    # 挂载配置文件及数据存放目录，启动端口为8080
    # docker run -d -p 8080:8081 -v /data/go-wiki/conf/:/opt/go-wiki/conf/ -v /data/go-wiki/data/:/data/go-wiki/data/ --name go-wiki go-wiki-image
    ```
## 2. 如果需要，可用 nginx 配置反向代理
```
upstream frontends {
    server 127.0.0.1:8088; # GO-Wiki 监听的ip:port
}
server {
    listen      80;
...
GO-Wiki 是站在巨人的肩膀上开发的一款软件，是因为系统中使用了非常多优秀的插件，非常感谢这些插件的作者们：
...
环境要求：go 1.8
```
$ git clone https://github.com/kamalyes/go-wiki.git
$ cd go-wiki
$ go build ./
```
