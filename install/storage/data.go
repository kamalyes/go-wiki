...
func createAdmin() (err error) {
	...
	defer db.Close()
	stmt, err := db.Prepare("INSERT gw_user SET username=?,password=?,given_name=?,role_id=?, create_time=?,update_time=?")
	if err != nil {
		return
	}
	defer stmt.Close()
...
func writeInstallData() (err error) {
	...
	// insert version
	stmt, err := db.Prepare("INSERT gw_config SET `name`=?,`key`=?,`value`=?,create_time=?,update_time=?;")
	if err != nil {
		return
	}
	defer stmt.Close()
...
func makeConf() (err error) {
	...
	logFilename := strings.Replace(filepath.Join(RootDir, "logs/go-wiki.log"), `\`, `/`, -1)
	templateConf = strings.Replace(templateConf, "#log.filename#", logFilename, 1)

	fileObject, err := os.OpenFile(filepath.Join(RootDir, "conf/go-wiki.conf"), os.O_RDWR|os.O_CREATE, 0777)
	if err != nil {
		return
	}
...
func installSuccess() {
	...
	if runtime.GOOS == "windows" {
		result["cmd"] = "go-wiki.exe --conf conf/go-wiki.conf"
	} else {
		result["cmd"] = "./go-wiki --conf conf/go-wiki.conf"
	}
	...
	go func() {
		...
		for {
			...
			case <-installChan:
				...
				// 开始安装
				log.Println("go-wiki start install")
				// 检查db
				err := checkDB()
				if err != nil {
