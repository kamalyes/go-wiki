...
func (up *Upgrade) v0ToV012() (err error) {
	...
	// INSERT INTO gw_privilege (name, parent_id, type, controller, action, icon, target, is_display, sequence, create_time, update_time) VALUES ('测试邮件服务器', 53, 'controller', 'email', 'test', 'glyphicon-list', 0, 80, unix_timestamp(now()), unix_timestamp(now()));
	...
	// 2. table gw_email add field 'is_ssl'
	// alter table gw_email add `is_ssl` tinyint(1) NOT NULL DEFAULT '0' COMMENT '是否使用ssl， 0 默认不使用 1 使用' after `password`
	db := G.DB()
	db.Exec(db.AR().Raw("alter table gw_email DROP COLUMN `is_ssl`"))
	_, err = db.Exec(db.AR().Raw("alter table gw_email add `is_ssl` tinyint(1) NOT NULL DEFAULT '0' COMMENT '是否使用ssl， 0 默认不使用 1 使用' after `password`"))
...
func (up *Upgrade) v012ToV013() error {
	...
	sql := "DROP TABLE IF EXISTS `gw_attachment`;" +
		"CREATE TABLE `gw_attachment` (" +
...
func (up *Upgrade) v013ToV018() error {
	...
	_, err := db.Exec(db.AR().Raw("alter table gw_log_document add `space_id` int(10) NOT NULL DEFAULT '0' COMMENT '空间ID'"))
	if err == nil {
		// 文档日志表里的 space_id
		_, err = db.Exec(db.AR().Raw("update gw_log_document as logDocment, gw_document as document set logDocment.space_id = document.space_id WHERE logDocment.document_id = document.document_id"))
		if err != nil {
			return err
		}
	}
	...
	_, err = db.Exec(db.AR().Raw("update gw_document set gw_document.sequence = gw_document.document_id WHERE sequence=0"))
	if err != nil {
		return err
	}
...
func (up *Upgrade) v018ToV020() error {
	...
	// 1. 配置表增加全文搜索开关
	insertSql := fmt.Sprintf("INSERT INTO `gw_config` (name, `key`, value, create_time, update_time) VALUES ('开启全文搜索', 'fulltext_search_open', '1', %d, %d)", updateTime, updateTime)
	_, err := db.Exec(db.AR().Raw(insertSql))
	if err != nil {
		return err
	}
	// 2. 配置表增加搜索索引时间间隔
	insertSql = fmt.Sprintf("INSERT INTO `gw_config` (name, `key`, value, create_time, update_time) VALUES ('索引更新间隔', 'doc_search_timer', '3600', %d, %d)", updateTime, updateTime)
	_, err = db.Exec(db.AR().Raw(insertSql))
	if err != nil {
		return err
	}
	// 3. 配置表增加系统名称配置
	insertSql = fmt.Sprintf("INSERT INTO `gw_config` (name, `key`, value, create_time, update_time) VALUES ('系统名称', 'system_name', 'Markdown Mini Wiki', %d, %d)", updateTime, updateTime)
	_, err = db.Exec(db.AR().Raw(insertSql))
	if err != nil {
		return err
	}
