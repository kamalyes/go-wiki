...
var (
	defaultConf = "conf/go-wiki.conf"

	confPath = flag.String("conf", "", "please set go-wiki conf path")

	version = flag.Bool("version", false, "go-wiki version")

	upgrade = flag.Bool("upgrade", false, "go-wiki upgrade")
...
` +
		"Author: kamalyes\r\n" +
		"Version: " + Version + "\r\n" +
		"Link: https://github.com/kamalyes/go-wiki"
	fg.Println(logo)
}
...
func initConfig() {
	...
	// init name
	beego.AppConfig.Set("sys.name", "go-wiki")
	...
	//beego.BConfig.WebConfig.Session.SessionName = "gowikissid"
...
func checkUpgrade() {
	if *upgrade == true {
		logs.Info("Start checking whether GO-Wiki needs upgrading.")
		...
		logs.Info("GO-Wiki Database version：" + versionDb)
		logs.Info("GO-Wiki Now version: " + Version)

		if versionDb == Version {
			logs.Info("GO-Wiki does not need updating.")
		} else {
			logs.Info("GO-Wiki start upgrading.")
			err := models.UpgradeModel.Start(versionDb)
			if err != nil {
				logs.Error("GO-Wiki upgrade failed.")
				os.Exit(1)
			}
			logs.Info("GO-Wiki upgrade finish.")
		}
		os.Exit(0)
	}
}
